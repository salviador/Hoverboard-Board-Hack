...
uint8_t connessione_stato;
extern volatile __IO struct TELEMETRY_JOYSTICK_dati telemetry_joystick_dati;
extern volatile __IO struct TELEMETRY_dati telemetry;
...
int main(void)
{
 ...
 //aci_gap_set_author_requirement(service_handle,1);
  ...
  while(1)
  {
      ...
      //Invio/Ricevo su UART valore joystick, collegato su motherboard
      connessione_stato = connected && bond_ok;
      Telemetry_TASK(connessione_stato);
      
      if(telemetry.dataREADY_JOYSTICK){
        if(connessione_stato){
          //Trasmetti a smartphone
            //Update Telemetry data
            if((HAL_GetTick() - time_Update_Telemetry) >= 100){       //Ogni 100mS se connesso invia telemetria
              time_Update_Telemetry = HAL_GetTick();
              
              telemetry_value.Battery = atof((char *)&telemetry.TelemetryRX_TramaBATTERIA[0]);
              telemetry_value.Current_m_L = atof((char *)&telemetry.TelemetryRX_TramaMOTORL[0]);
              telemetry_value.Current_m_R = atof((char *)&telemetry.TelemetryRX_TramaMOTORR[0]);
...
void JoyStick_notify_Callback(int x, int y){
  ...
  telemetry_joystick_dati.dataREADY_JOYSTICK = 1;
  telemetry_joystick_dati.joyx = x;
  telemetry_joystick_dati.joyy = y;  
}
